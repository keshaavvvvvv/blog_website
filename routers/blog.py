...
def submit_blog_form(request: Request,title: str = Form(...),body: str = Form(...) ,db: Session = Depends(database.get_db),image:UploadFile = None):
...
async def update_blog(blog_id: int, title: str = Form(...), body: str = Form(...),image:UploadFile = None, db: Session = Depends(database.get_db)):
    blog = db.query(models.Blog).filter(models.Blog.id == blog_id).first()
    if not blog:
        raise HTTPException(status_code=404, detail="Blog not found")
    blog.title = title
